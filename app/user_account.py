...
class user_account:
    ...
    def isProvidingLiqBALN(self, address) -> bool:
        isLP = 0 #network.isEarningRewards(address, 3) or network.isEarningRewards(address, 4)
        return bool(isLP)

    def updateUser(self, address, day):
        self.address = address
        self.balance = self.balanceData(address)
        self.isLP = 0 #self.isProvidingLiqBALN(address)
        ...
        total_dividents = network.getUnclaimedDividends(address)
        ...
        self.totalBALNdividents = int(total_dividents[0][network.BALN_CONTRACT], 16)/EXA
        self.totalBNUSDdividents = int(total_dividents[0][network.BNUSD_CONTRACT], 16)/EXA
        self.totalSICXdividents = int(total_dividents[0][network.SICX_CONTRACT], 16) /EXA
        return True
